...
const repeatedInput = Buffer.from(
    "Brotli benchmark data. ".repeat(Math.ceil(size / 23)).slice(0, size),
);
...
console.log(
    `Repeated: ${repeatedInput.length} bytes -> ${repeatedCompressed.length} bytes compressed`,
);
